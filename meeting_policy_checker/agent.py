...
def check_meeting(
    ...
    """
    Validates inputs, builds prompt, calls OpenAI, validates output.
    Returns OutputPayload. On failure, returns needs_review with reason.
    """
    ...
    truncation_note = "Input truncated to size limits." if (policy_trunc or meeting_trunc) else None
...
3. Output ONLY JSON strictly matching this schema (no extra keys, all keys present):
{
  "summary": {
...
- If unsure, set summary.overall to "needs_review".
...
"""
    ...
    # --- Improvement: infer org_context from "Confidentiality level" if missing ---
    org_context = payload.org_context.dict() if payload.org_context else None
    if not org_context:
        conf_match = re.search(r"Confidentiality level\s*:\s*(\w+)", meeting, re.IGNORECASE)
        if conf_match:
            org_context = {"value": conf_match.group(1).strip()}
    ...
    def _call(response_format_json: bool = True):
